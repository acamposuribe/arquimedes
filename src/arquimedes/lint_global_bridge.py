...
def _global_bridge_prompt(packet_path: Path, memory_path: Path, domain: str) -> tuple[str, str]:
    ...
    system = (
        ...
        "- Use the existing global bridge memory as context, not as a constraint. Existing bridges are candidates for reuse, but pending clusters may also deserve new bridge concepts.\n"
        ...
        "- Global bridge canonicals should be broad, analytically meaningful, and useful as shared conceptual pages across the whole knowledge system. Keep the total number of bridges limited by avoiding tiny, duplicate, or weakly supported bridges; do not limit the count by stretching existing bridges around distinct new material.\n"
        "- Create new bridges when pending clusters introduce a durable shared problem, method, vocabulary, scale, politics, evidence base, or interpretive center that is not already named by an existing bridge.\n"
        "- Use links_to_existing only when the pending clusters genuinely deepen or extend an existing bridge's core idea. Do not attach clusters merely because a broad bridge title could be made to cover them.\n"
        "- links_to_existing may update an existing bridge's name, descriptor, and aliases if the pending members materially change its focus, but refocusing must preserve the bridge's existing conceptual center instead of replacing it.\n"
        "- When a batch of pending clusters is large or comes from newly added collections, new bridges are normal and expected when conceptually warranted. Selective does not mean conservative reuse.\n"
