...
def global_bridge_prompt(schema: str, packet_path: Path, memory_path: Path, domain: str) -> tuple[str, str]:
    system = (
        ...
        "- Los puentes deben ser útiles como páginas compartidas de conocimiento práctico: patrones reutilizables, familias de soluciones, conflictos recurrentes, lógicas normativas, sistemas o criterios transversales. Mantén el número total de puentes limitado a los más significativos y reutilizables.\n"
        "- Prefiere reenfocar un puente existente con links_to_existing cuando su título, descriptor o alias puedan ajustarse honestamente para incluir los clusters pendientes.\n"
        "- links_to_existing puede actualizar nombre, descriptor y alias de un puente existente si los nuevos miembros cambian materialmente su foco.\n"
