...
def workspace_freshness_status() -> dict:
    ...
    if status["repo_dirty"]:
        status["message"] = "Repository has local changes; update blocked."
...
def update_workspace() -> dict:
    ...
    if status["repo_applicable"] and not status["repo_dirty"] and status["has_upstream"]:
        ...
        if pull.returncode == 0:
            ...
            status["message"] = (pull.stderr or pull.stdout or "git pull failed").strip()
    elif status["repo_dirty"]:
        status["pull_result"] = "blocked"
