...
### LLM-driven passes:
- **Cluster audit**: review bridge clusters incrementally, but allow merges, splits, renames, and broader reorganization when new evidence reveals a better cross-material structure; the goal is a stronger current bridge graph, not preservation for its own sake. Deterministically invalid edits, such as shrinking a cluster below two distinct materials, should be ignored without aborting the rest of the audit batch.
...
### Step 2 rollout pass:
- **Global bridge stage**: incrementally clusters collection-local clusters into `derived/global_bridge_clusters.jsonl`, using compact hosting-collection context plus existing global bridge memory enriched with connected local-cluster reflections and collection signals; the same pass also emits bridge-level synthesis fields, with `why_this_bridge_matters` serving as the page-grade mini-essay for each bridge, skips entirely when fewer than two collections are in scope, and is currently invoked through `arq lint --stage global-bridge` while the rest of the Step 2 graph migration lands incrementally
