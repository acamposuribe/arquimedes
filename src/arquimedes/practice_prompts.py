...
def global_bridge_prompt(schema: str, packet_path: Path, memory_path: Path, domain: str) -> tuple[str, str]:
    system = (
        ...
        "- Esta pasada solo decide membresía de puentes. No escribas reflexiones, ensayos, takeaways, tensiones, preguntas ni fuentes recomendadas aquí.\n"
        "- Usa la memoria de puentes existentes como contexto, no como restricción. Los puentes existentes son candidatos de reutilización, pero los clusters pendientes también pueden merecer puentes nuevos.\n"
        ...
        "- Los puentes deben ser útiles como páginas compartidas de conocimiento práctico: patrones reutilizables, familias de soluciones, conflictos recurrentes, lógicas normativas, sistemas o criterios transversales. Mantén el número total de puentes limitado evitando puentes pequeños, duplicados o débilmente sostenidos; no limites el número estirando puentes existentes para absorber material distinto.\n"
        "- Crea puentes nuevos cuando los clusters pendientes introduzcan un problema, método, vocabulario, escala, sistema, familia de soluciones, criterio normativo o centro práctico que no esté ya nombrado por un puente existente.\n"
        "- Usa links_to_existing solo cuando los clusters pendientes profundicen o extiendan de verdad la idea central de un puente existente. No los enlaces solo porque un título amplio pueda hacerse encajar.\n"
        "- links_to_existing puede actualizar nombre, descriptor y alias de un puente existente si los nuevos miembros cambian materialmente su foco, pero el reenfoque debe conservar su centro conceptual existente en vez de sustituirlo.\n"
        "- Cuando el paquete pendiente sea grande o venga de colecciones nuevas, los puentes nuevos son normales si están conceptualmente justificados. Ser selectiva no significa reutilizar de forma conservadora.\n"
    ...
    user = (
        ...
        "Trata ambos archivos como material fuente para esta pasada de clustering.\n"
        "Usa las reflexiones compactas de clusters locales y las señales de colección solo para decidir membresía.\n"
